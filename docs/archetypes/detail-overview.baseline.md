...
- Header fill → `headerFill.ts` / `HeaderFillContext` contract: `solid` (accent-filled, `bg-primary` + inverted bar text, default) / `tint` (`bg-muted` + hairline border) / `white` (`bg-card` + hairline border); set ONCE via `<AppShell headerFill>`. The Mode B bar is the shared `<SurfaceHeaderBar>` (`src/components/layout/SurfaceHeaderBar.tsx`), which reads the context and applies `headerFillClasses(fill).bar` — the shell never reads the class table itself. There is no per-shell override — the context is the only entry point (hard rule 12). Mode A's `<DetailOverviewHeader>` is a bare `<PageHeader>` with no fill.
...
- Mode B framed header bar → the shell's `title` presence renders the shared `<SurfaceHeaderBar>` (canonical `px-5 py-4`, the `hfc.bar` fill from the `HeaderFillContext` set once at `<AppShell headerFill>`), wrapping the `<NestedPageHeading>`. No `headerFill` prop on the shell, no `<DetailOverviewHeader>` call, no per-shell padding or class-table read.
