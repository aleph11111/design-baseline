...
- The generated page imports only its own `design-baseline/archetypes/<slug>` and consumer-local paths (react, react-hook-form, the project's `@/components/ui/*` alias). It does not import `design-baseline/ui/*`: consumers reach ui through that alias (project-first, package fallback, per `docs/PACKAGE.md` wiring line 2), and a direct package import would bypass a project override. It renders the loading, empty and error branches, except where the contract puts a plane elsewhere: detail-overview and matrix-grid (route-owned loading and error), form-page (initial values resolve before render; no empty plane), import-wizard (no page-level fetch) and calendar (empty days use `emptyDayLabel`). `scripts/new-page.test.mjs` enforces both the planes and this exemption list, and `docs/PACKAGE.md` mirrors it.
