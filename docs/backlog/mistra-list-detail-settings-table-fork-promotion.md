...
- [ ] `hideBelowMd?: boolean` on `TableColumn` (used ~17 call sites in mistra: ActionItemsTable, HealthPage,
...
- [ ] `footer?: ReactNode` on `ListWithDetailShellProps`, rendered as a band inside the card below the body
...
- [ ] `emptyStateAction?: ReactNode` on `ListWithDetailShellProps`, threaded to `ListWithDetailEmptyState`'s
...
- [ ] `RowAction<Row>.label` and `.disabled` accepting `T | (row: Row) => T` (mistra's per-row toggle label
...
- [ ] `rowLabel: string | ((count: number) => string)` on `SettingsTableShellProps` for the result-count
...
- [ ] For every feature that ships: bump `docs/archetypes/list-with-detail.md` and/or
      ...
      change).
- [ ] Run `node scripts/verify-exports.mjs` and `node scripts/verify-manifest-versions.mjs` green, then cut
