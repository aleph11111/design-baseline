...
| `docs/ADOPTION.md` | **Methodology** (v1.2) — the adoption contract: 9-point checklist (point 1 is *frame adopted* — use `AppShell`, no raw `<main>`), 4-gate enforcement stack, two-way feedback loop. Its mechanical gate 2 ships as the zero-dep `scripts/lint-design.mjs` + `_adherence.json` at repo root |
