...
- **Page scaffolder** — the package ships a bin, `design-baseline new-page <archetype> <Name>` (`bin/new-page.mjs`, zero-dep), which writes `<Name>Page.tsx` from `templates/<archetype>.tsx`: one minimal page per page archetype a consumer uses, importing only its own `design-baseline/archetypes/<slug>` plus consumer-local paths and rendering loading / error / empty (route-owned for detail-overview and matrix-grid, per their contracts). Route registration stays consumer-owned via `--register <cmd>`. `scripts/new-page.test.mjs` generates every template and runs `tsc` on it, so a shell API change that breaks a template fails `npm test`. Templates are added on first consumer need, not for every page archetype.
